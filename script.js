...
function one(x) {
    ...
    function two() {
        ...
        if (h > 0) {
            if ((a > x) && (a > 0)) {alert("Загаданное число меньше, осталось попыток " + h); return two();};
            if ((a < x) && (a > 0)) {alert("Загаданное число больше, осталось попыток " + h); return two();};
        };

        if (h === 0) {
            f = +confirm("Попытки закончились, хотите сыграть еще?"); 
